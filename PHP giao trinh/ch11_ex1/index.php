...
switch( $action ) {
    ...
    case 'Modify Task':
        $task_index = filter_input(INPUT_POST, 'taslid', FILTER_VALIDATE_INT);
        if ($task_index === NULL || $task_index === FALSE) {
            $errors[] = 'The task cannot be deleted.';
        } else {
    ...
    case 'Save Changes':
        $i = filter_input(INPUT_POST, 'taskid', FILTER_VALIDATE_INT);
        $modified_task = filter_input(INPUT_POST, 'modified_task');
        if (empty($modified_task)) {
            $errors[] = 'modified_task empty.';
        } else if ($i = NULL || $i == FALSE) {
            $errors[] = 'task cannot be modify';
        } else {
